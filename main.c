...
void low_priority interrupt externInterrupt0(void)
{
    if (INTCON3bits.INT1IE && INTCON3bits.INT1IF) {
        ...
        PIR1bits.TMR1IF = 0;
        T1CONbits.TMR1ON = 1;
        mode = (mode + 1) % NUM_OF_MODES; // 0,1,2,3,4   5種循環
    } else if (PIE1bits.TMR1IE && PIR1bits.TMR1IF) {            //用timer1做INT1的開關,按下按鈕後就先把INT1的interrupt先關閉,0.35秒後再打開
        PIR1bits.TMR1IF = 0;                                //,才不會按一次就一直進到interrupt切換mode
        PIE1bits.TMR1IE = 0;
        T1CONbits.TMR1ON = 0;
...
void nmea_read(void)
{
    ...
    buf[line_idx][idx++] = c;   //沒吃到'\n'之前 把RCREG的char放進buffer
}
            ...
            if (isOK_1 && isOK_2) {
                ...
                if (info.fix) {
                    ...
                    case 0:
                        ...
                        sprintf(oled_buf, "%7.2f km/h", loc.speed * 1.852f); //海里轉公里
                    ...
                    case 3: {
                        ...
                        lat_min_frac = fmod(loc.latitude, 1.0);     //度分分.分分分分剩下小數點後的 .分分分分
                        lat_sec = (int) (60.0f * lat_min_frac);     //乘上60變為經秒
                        ...
                        sprintf(oled_buf, "  %2d %2d'%2d\"%c", lat_deg, lat_min, lat_sec, lat); //lat為extern int 宣告的變數會在gps.c被定義
                        oled_buf[4] = 0xdf; //度的符號 小圈圈
                        oled_write_upper_line(oled_buf);
                    ...
                    if (loc.HDOP < 2.1 && info.satellites >= 6) {
                        if (loc.speed > 1.079913)  // around 2 km/h
                            distance += loc.speed * 0.5144445f;　//轉成 公尺
                    }
                } else {
                    oled_write_upper_line(starbrust);
                    oled_write_lower_line(stream);
